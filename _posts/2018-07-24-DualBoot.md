...
Turn on/reboot the computer and enter BIOS.  Change the boot order so that the USB is first.  Many laptops now have a "secure boot" feature.  This means that the computer will only boot OS's that are signed with a key.  Ubuntu is compatible with secure boot, but if you want external drivers, it's best to disable secure boot.  Usually, the option to disable secure boot is greyed out, so delete all of the keys involved with secure boot, then disable secure boot.
